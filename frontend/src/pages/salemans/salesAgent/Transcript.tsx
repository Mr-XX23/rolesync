...
const UserBubble: React.FC<{ text: string; sending?: boolean }> = ({ text, sending = false }) => {
  const { body, names } = splitAttachmentNote(text);
  return (
    <div className="flex justify-end">
      ...
      >
        {body}
        {names.length > 0 && (
          <ul className={`flex flex-wrap gap-1.5 ${body.trim() ? 'mt-2 pt-2 border-t border-primary-foreground/20' : ''}`}>
            {names.map((name) => (
              <li
                key={name}
                className="inline-flex items-center gap-1 rounded-md bg-primary-foreground/15 px-1.5 py-0.5 font-mono text-[10px] max-w-[14rem]"
              >
...
export const Transcript: React.FC<{
  ...
  <div className="space-y-4">
    {items.map((item, index) => {
      switch (item.kind) {
              ...
              : tool === DELEGATE_TOOL
                ...
                : MEMORY_TOOLS.has(tool)
                  ...
                  : WRITE_TOOLS.has(tool)
                    ...
                    : Search;
          return (
            <div key={index} className="flex items-center gap-2 text-xs text-muted-foreground pl-10">
        ...
        case 'tool_result': {
          ...
          return (
            <div key={index} className={`ml-10 text-xs rounded-xl border px-3 py-2 ${tone}`}>
    ...
    {draft && <AssistantMessage text={draft} streaming />}
