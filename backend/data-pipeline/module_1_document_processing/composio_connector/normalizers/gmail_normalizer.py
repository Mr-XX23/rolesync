...
def normalize_gmail(payload: dict[str, Any], tenant_id: str = "tenant_default") -> CanonicalEvent:
    ...
    thread_id = data.get("threadId") or data.get("thread_id") or message_id
    user_id = meta.get("user_id") or payload.get("user_id") or data.get("user_id") or "usr_active"
    connected_account_id = (
        meta.get("connected_account_id")
        or (meta.get("connected_account", {}).get("id") if isinstance(meta.get("connected_account"), dict) else None)
    ...
    preview_body = data.get("preview", {}).get("body") if isinstance(data.get("preview"), dict) else ""
    preview_subj = data.get("preview", {}).get("subject") if isinstance(data.get("preview"), dict) else ""
    ...
    raw_attachments = data.get("attachmentList") or data.get("attachments") or []
    ...
    subject = data.get("subject") or preview_subj or "No Subject"
    ...
    return CanonicalEvent(
        ...
        metadata={
            ...
            "body": body_text,
            "snippet": data.get("snippet") or preview_body[:200] if preview_body else "",
            "label_ids": data.get("labelIds") or data.get("label_ids") or ["INBOX"],
            "thread_id": thread_id,
            "message_id": message_id,
