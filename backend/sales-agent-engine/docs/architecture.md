...
- **Knowledge base.** No retrieval endpoint exists: vector search is a placeholder, embeddings are fake, there is no pgvector, and data-pipeline's document search only matches metadata. The KB adapter (`tools/adapters/knowledge.py`) ranks the user's documents by their sales metadata, reads the best candidates' text, and returns matching passages; real retrieval would replace only that file. Scoping follows data-pipeline as it is today. Vault documents are all stored under the tenant `"tenant_default"`, because the UI never sends a workspace, so the engine filters by user. Reads by document id don't check ownership in data-pipeline, so the engine checks the document is in the user's own list first.
- **Catalog.** It is real (Postgres, scoped by workspace UUID) and served by data-pipeline, not a separate service. Its "semantic" search is weighted keyword scoring. The adapter uses it for ranking and falls back to the keyword product filter on older data-pipeline builds without it (the running image predated it on 2026-09-11). Prices and discount bounds arrive as decimal strings and are passed on unchanged.
