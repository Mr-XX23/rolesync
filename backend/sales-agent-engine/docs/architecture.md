...
- **Write tools (Phase 3, decided 2026-09-12).** Calendar events, Slack messages and Notion pages go through Composio. Generated documents (docx, pptx, xlsx, pdf, md, rendered in the engine) and quotes are saved to the rep's **Google Drive** (Composio file upload); if Drive isn't connected or is full they go to the **workspace knowledge vault** instead (data-pipeline now also accepts PPTX and XLSX uploads), and the chat links to the file. Catalog writes use data-pipeline's catalog API: create items, update details, prices and discount limits, set stock, reserve and release stock, and retire (never hard-delete). Viewers of a workspace can't change its catalog. Quotes price SKUs from the catalog, enforce each item's `max_discount_pct`, and can reserve the quoted stock.
