...
- **Knowledge base.** The vault is shared by a workspace, like the catalog: data-pipeline checks the caller is a member of `X-Tenant-Id` (asking workspace-service, cached 60s), records the uploader, and lets only the uploader or an OWNER/ADMIN delete. Search uses data-pipeline's semantic search (`POST /knowledge-vault/search`, pgvector HNSW with the workspace and the caller's ACL as filters, each hit carrying its wider parent context). The KB adapter (`tools/adapters/knowledge.py`) groups hits by document and names them from the list of indexed documents; when semantic search is unavailable or finds nothing (older documents may have no vectors) it falls back to ranking documents by their sales metadata and reading the best candidates' text.
- **Knowledge-base changes (decided 2026-09-14).** The agent can do what the vault page does, each after approval (`tools/adapters/knowledge_writes.py`): add a public web page (`ingest-url`; the same address again refreshes it, and the engine sends the name and category it already has so a refresh doesn't rename or reclassify it), correct a document's classification, have it classified or indexed again, and delete a document. These tools need the KNOWLEDGE scope, which only the coordinator has, so sub-agents read the knowledge base but never change it. The engine checks what data-pipeline would refuse before the approval card appears: viewers change nothing, a member deletes only their own documents, a page address must not point inside the network, and a document still being processed isn't classified, indexed again or deleted (deleting mid-processing can leave its index behind, because the ingestion job keeps writing). Undo deletes a page the agent added, first waiting up to 40 s for its processing to finish, and restores a classification's previous values; a deleted web page is added back from its address, and a deleted file can't be restored. data-pipeline's `ingest-url` fetches only public addresses: it resolves the host itself, refuses loopback, private, link-local and other non-global answers, and connects to the address it checked, on redirects too (`module_1_document_processing/pipeline/safe_fetch.py`). Re-indexing a document or re-crawling a page now also clears its chunk fingerprints; removing only the vectors had made the delta check skip unchanged content, leaving the document in Error with no chunks.
...
- **Deals (Phase 4, decided 2026-09-12).** Deals live in **workspace-service** (`workspace_deals`, shared by the workspace, JPA `@Version`): `GET/PUT/DELETE /api/v1/workspaces/{id}/deals[/{dealId}]`. Ids are chosen by the caller, so an agent's retried create can't make a second deal; a write may carry `expected_version` and is refused with 409 when someone changed the deal first; deleting is limited to the owner or a workspace OWNER/ADMIN. The engine's `search_deals` / `create_deal` / `update_deal` call that API: an update that hits 409 re-reads and re-applies only its own fields, and its undo restores only the fields nobody has touched since. Quotes carry `deal_id` and are listed on the deal. The pipeline board is `/salesman/deals` in the frontend.
- **Write tools (Phase 3, decided 2026-09-12).** Calendar events, Slack messages and Notion pages go through Composio. Generated documents (docx, pptx, xlsx, pdf, md, rendered in the engine) and quotes are saved to the rep's **Google Drive** (Composio file upload); if Drive isn't connected or is full they go to the **workspace knowledge vault** instead (data-pipeline now also accepts PPTX and XLSX uploads), and the chat links to the file. Catalog writes use data-pipeline's catalog API: create items, update details, prices and discount limits, record stock movements, reserve and release stock, and retire (never hard-delete). Since data-pipeline's typed stock movements (2026-09-13) the ledger is append-only: the agent records what happened (received, sold, shipped, damaged, lost, returned), and only workspace OWNER/ADMIN may replace a count, with a note and the on-hand figure it was based on (409 if stock moved since); the engine checks the role before asking for approval. Viewers of a workspace can't change its catalog. Quotes price SKUs from the catalog, enforce each item's `max_discount_pct`, and can reserve the quoted stock.
- **Compensation asks first (decided 2026-09-12).** There is no automatic rollback. Undoing is a gated write, `undo_actions`: when a write FAILED, gave no answer, or its approval expired after other actions of the same request succeeded, the orchestrator's `compensate` node proposes undoing those actions; the approval card lists what would be undone and what can't be (a sent email). Each tool's handler records how to reverse what it did when it runs (`saga_step.undo_action`, e.g. the created event's id, the stock level it replaced); undo steps run newest first and end COMPENSATED or COMPENSATION_FAILED with their own `undo:<tool>` audit rows. `session.turn` / `saga_step.turn` scope an automatic offer to the current request; the model can also undo earlier actions by the `action_id` in their results when the rep asks.
- **Context and memory (Phase 4, decided 2026-09-12).** Memory lives in the engine (`agent.memory`), one versioned row per (workspace, scope, key) with optimistic locking: a write inserts the next version, and a writer that loses the race re-applies its change to the winner's content instead of overwriting it. Scopes: CONVERSATION (a session's running summary), REP (private to the rep it is about), ACCOUNT and DEAL (customer knowledge shared with the workspace, closed to viewers for writing). The agent saves and deletes memories itself, without approval but audited, and the rep reviews and deletes them under "What the agent remembers" (`api/memory.py`). Account keys normalize a company name (`Acme Corp.`, `ACME, Inc` both key on `acme`). Tool results too large for the prompt are offloaded whole to `agent.context_blob` and referenced in the conversation, and `read_offloaded_result` reads them back. The rep's persona and communication style come from workspace-service's profile, cached.
- **Sub-agents (Phase 5, decided 2026-09-12).** Three: research (reads only), outreach (reads + communication) and quote (reads + catalog + documents). The orchestrator hands work over with the `delegate` tool, which only it has the scope for, so a sub-agent cannot start another (nor change the knowledge base, which needs the coordinator-only KNOWLEDGE scope); each sub-agent's tools come from `registry.SCOPES` and are enforced at the gate, which audits every call under the agent that made it. A sub-agent's conversation is kept in the run's graph state (`delegations`), so a write inside it pauses for approval and resumes exactly where it stopped, and the approval card names the sub-agent. Only the sub-agent's result goes back to the planner (with a short log of what it did and its sources), never its steps. Read-only sub-agents take their model steps together; anything that can pause runs alone. The orchestrator keeps its own tools as well, so simple one-step actions don't need a delegation round trip.
- **Guardrails.** Per call (`tools/executor.py`): timeout, bounded retries with backoff for transient failures of reads and undo steps (writes never retry), and a per-tool circuit breaker. Per turn (`engine/guardrails/limits.py`): model steps, tool calls, tokens, and identical-call loop detection; a breach ends the turn HALTED with a `halted` event. The runner's sweep expires approvals past their TTL and resumes the session with that decision. Per workspace (`engine/guardrails/budgets.py`, Redis): concurrent runs, requests per user per minute, and model tokens per day, checked before a turn starts.
